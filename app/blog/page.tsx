          ...
          {posts.map((post, i) => (
            <AnimatedSection key={post.slug} delay={i * 0.08}>
              <article className="border-b border-white/10 pb-12">
                <time className="text-sm text-gray-500 tracking-wide uppercase">
                  {format(new Date(post.date), 'MMMM d, yyyy')}
                </time>
                <h2 className="font-serif text-3xl text-white mt-2 mb-3">
                  <Link href={`/blog/${post.slug}`} className="hover:text-brand-500 transition-colors">
                    {post.title}
                  </Link>
                </h2>
                ...
                <div className="flex items-center gap-4">
                  {post.tags?.map(tag => (
                    <span key={tag} className="text-xs font-medium bg-white/10 text-gray-300 px-3 py-1 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
