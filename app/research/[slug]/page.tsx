  ...
  return (
    <div className="min-h-screen bg-ocean-dark text-white pt-24 pb-32">
      <article className="max-w-4xl mx-auto px-6">
        <Link
          href="/#research"
          className="text-[#4ecdc4] text-sm hover:underline mb-12 block relative z-10"
        >
          ← Back to Research
        </Link>
            ...
            {item.tags && (
              <div className="flex flex-wrap gap-2 mt-6">
                {item.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs font-medium bg-white/10 text-white/80 px-3 py-1.5 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
