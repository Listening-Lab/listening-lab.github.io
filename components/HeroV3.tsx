...
export default function HeroV3() {
  return (
    <section className="relative min-h-[90vh] bg-ocean-dark text-white overflow-hidden flex items-center">
      ...
      {/* ── Blend layers (applied on top of image) ── */}

      {/* 1. Overall dark tint — pulls the image toward ocean-dark */}
      <div className="absolute inset-0 bg-ocean-dark/55" />

      {/* 2. Left-to-right gradient — solid ocean-dark on the left, fading out */}
      <div
        className="absolute inset-0"
        style={{
          background:
            'linear-gradient(to right, #0a1628 30%, rgba(10,22,40,0.85) 52%, rgba(10,22,40,0.3) 75%, transparent 100%)',
        }}
      />

      {/* 3. Subtle blue colour wash — ties the photo hues to the brand palette */}
      <div
      ...
      {/* 4. Top vignette — keeps the header area dark */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            'linear-gradient(to bottom, rgba(10,22,40,0.5) 0%, transparent 30%)',
        }}
      />

      {/* 5. Bottom fade — transitions cleanly into the next section */}
      <div
      ...
      {/* ── Left: text content (unchanged from HeroV2) ── */}
